...
func (t *Uint64Tree) Insert(key uint64, value any) {
	t.g.Insert(key, value)
}

// Search returns the value associated with key from the tree.
func (t *Uint64Tree) Search(key uint64) (any, bool) {
...
func (t *Uint64Tree) Update(key uint64, callback func(any, bool) any) {
...
func (t *Uint64Tree) NewScanner(key uint64) *Uint64Cursor {
	return &Uint64Cursor{t.g.NewScanner(key)}
}
...
// modification of the locked node. The leaf node is only unlocked after
// closing the Cursor.
func (t *Uint64Tree) NewScannerAll() *Uint64Cursor {
	return &Uint64Cursor{t.g.NewScannerAll()}
}

// Uint64Cursor is used to enumerate key-value pairs from the tree in
// ascending order.
type Uint64Cursor struct {
	g *GenericCursor[uint64, any]
}
...
func (c *Uint64Cursor) Close() error { return c.g.Close() }
...
func (c *Uint64Cursor) Pair() (uint64, any) { return c.g.Pair() }
...
// the Pair method.
func (c *Uint64Cursor) Scan() bool { return c.g.Scan() }
